...
import { cancelCourseByMaKhoaHoc, joinCourseByMaKhoaHoc } from "@/services/user/courses/course.service";
...
export default function CourseCardItem({ course, isJoined }: { course: Course, isJoined: boolean }) {
  ...
  return (
    <Card className="cursor-pointer w-full">
      <Link href={`/course/${course.maKhoaHoc}`}>
        <CardBody
          className="p-0 overflow-hidden shrink-0 flex items-center justify-center h-[180px]"
        >
          <SafeImage
            src={course.hinhAnh}
